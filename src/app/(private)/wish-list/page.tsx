...
export default function WishList() {
    ...
    return (
        <div className="p-5">
            <NavigationPath />
            <BackNavigation />
            <h1 className="font-lato text-[59.88px] font-extrabold mt-[10px] ml-[112px]">
                WishList
            </h1>
                    ...
                    <tbody>
                        {wishlistItems.map((item) => (
                            <tr key={item.id} className="hover:bg-gray-50">
                                ...
                                <td
                                    className={`py-4 px-4 font-['Public Sans'] text-[13.45px] font-semibold uppercase whitespace-nowrap ${item.stockStatus === "In Stock" ? "text-[#2DB224]" : "text-[#EE5858]"
                                        }`}
                                >
                                    {item.stockStatus}
                                </td>
                                <td className="py-4 px-4 h-[47px] w-[240px]">
                                    <div className="flex items-center justify-between">
                                        <button
                                            disabled={item.stockStatus !== "In Stock"}
                                            onClick={() => handleAddToCart(item)}
                                            className={`group relative flex items-center rounded-[5px] justify-center gap-2 font-['Public Sans'] text-[13.45px] font-semibold uppercase whitespace-nowrap h-[47px] w-[167px] overflow-hidden ${item.stockStatus === "In Stock"
                                                ? "bg-[#1877F2] hover:text-[#FFFFFF] transition-colors duration-300 cursor-pointer"
                                                : "bg-[#ADB7BC] cursor-not-allowed"
                                                } text-[#FFFFFF]`}
                                        >
                                            ...
                                            {item.stockStatus === "In Stock" && (
                                                <span
                                                    className="absolute inset-0 bg-[#29c9d7] transform -translate-x-full group-hover:translate-x-0 transition-transform duration-300 ease-out"
                                                ></span>
                                            )}
                                        </button>
                                        <IoCloseCircleOutline
